...
export class ViewImageComponent implements OnInit, OnDestroy {
  ...
  public addImageComment(){
   ...
   setTimeout(() => {
     ...
     this.commentsArrayReversed.unshift(adjustedImageCommentObj)
  ...
  public deleteImageComment(comment: ImageComment){
    ...
    if(confirm('Are you sure you want to delete this comment?')){
      ...
      setTimeout(() => {
        ...
        const showingImageInGalleryObject = this._gallery.images.find(image => image.id === this._imageID)
        showingImageInGalleryObject.comments = showingImageInGalleryObject.comments.filter(c => c !== comment)
